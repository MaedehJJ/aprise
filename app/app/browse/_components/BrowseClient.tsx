          ...
          <div className="space-y-8 max-w-2xl">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {browseResult.jds.length} JD
                {browseResult.jds.length !== 1 ? "s" : ""},{" "}
                {browseResult.resumes.length} resume
                {browseResult.resumes.length !== 1 ? "s" : ""}
              </span>
            </div>
...
function ResumeCard({
  ...
  const tags: string[] = (labels.tags as string[]) ?? [];
  const roleInfo = [labels.role_focus, labels.domain].filter(Boolean).join(" · ");
  ...
  return (
    <div className="group flex items-start gap-3 p-3.5 rounded-xl border border-border/60 bg-card hover:border-border hover:shadow-sm transition-all">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground">Generated resume</p>
        {roleInfo && (
